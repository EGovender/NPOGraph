...
- [ ] **Milestone 5 — Design tool expansion:** a staged/sectioned questionnaire; questions covering every Phase 3.5 arrangement type and multi-role organizations (today only one of thirteen questions touches this layer, and it only ever adds `Fiscal Sponsor`); rendering the already-modeled-but-unused per-question `help` text; export (JSON/JSON-LD/Markdown); a shareable URL; "open in graph."
